...
impl<T> Drop for DropTracker<T> {
    fn drop(&mut self) {
        // let mut mutable_borrow = counter.borrow_mut();
        // *mutable_borrow+=1;
        // drop(mutable_borrow);
        // we can just dereference it without assigning to a variable
        // the lifetime of this mutable borrow is contained within
