...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let ticket_res = Ticket::new(title.clone(), description, status.clone());

    match ticket_res {
        Ok(t) => t,
        Err(TicketNewError::TitleError(err)) => panic!("{err}"),
        Err(TicketNewError::DescriptionError(_)) => Ticket{ title, description: "Description not provided".into(), status},
    }
}
